  ...
  return (
    <div className="px-4 pb-[140px] pt-4 flex flex-col gap-4">
      ...
      {displayCategories.length === 0 ? (
        <div className="rounded-[20px] border border-dashed border-border bg-surface p-8 flex flex-col items-center gap-3 text-center">
          <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center">
            <DraftingCompass className="w-6 h-6 text-primary" />
          </div>
          <p className="font-semibold text-text text-sm">Brėžinių dar nėra</p>
        ...
        displayCategories.map((category) => {
          ...
          return (
            <div key={category} className="rounded-[20px] border border-border bg-surface overflow-hidden shadow-card">
              <div className="flex items-center gap-2 px-4 py-3 bg-surface-2 border-b border-border">
                ...
                <span className="font-semibold text-text text-sm truncate">{category}</span>
              </div>
              ...
              <div className="p-3">
                {file && isPreviewable(file.name) ? (
                  ...
                  >
                    ...
                    {!busy && <span className="text-muted text-xs">PNG, JPG · spausk norėdamas įkelti</span>}
                  </button>
                )}
              </div>
            </div>
      ...
      {showCategoryModal && (
        <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-surface rounded-[20px] shadow-card w-full max-w-sm overflow-hidden border border-border">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <h3 className="text-[15px] font-bold text-text">Nauja kategorija</h3>
              <button
                onClick={() => setShowCategoryModal(false)}
                className="text-muted active:text-text transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
