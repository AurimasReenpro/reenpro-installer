  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div
        ...
        className="bg-surface rounded-[16px] border-2 border-dashed border-border/60 dark:border-white/10 hover:border-primary/50 hover:bg-surface-2/20 transition-all cursor-pointer p-8 flex flex-col items-center gap-3 group"
      >
        {uploadMutation.isPending ? (
          <Loader2 className="w-8 h-8 text-primary animate-spin" />
        ) : (
          <div className="w-12 h-12 rounded-[12px] bg-surface-2 dark:bg-surface-2 group-hover:bg-surface-2 transition-colors flex items-center justify-center">
            <Upload size={22} className="text-subtle dark:text-subtle group-hover:text-primary transition-colors" />
          </div>
        )}
      ...
      {filesLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 gap-2 bg-surface rounded-[16px] border border-border/20 dark:border-white/10 shadow-sm">
          <FolderOpen size={32} className="text-subtle mb-2" />
          <p className="text-subtle dark:text-subtle text-[14px]">Failų dar nėra.</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {regularFiles.map((file) => (
            <motion.div
              ...
              className="group relative bg-surface rounded-[12px] border border-border/20 dark:border-white/10 shadow-sm overflow-hidden hover:border-primary/30 transition-colors"
            >
              {isImage(file.name) ? (
                <div className="aspect-[4/3] overflow-hidden bg-surface-2 dark:bg-surface-2">
                  <img
                    src={file.url}
                    alt={file.name}
                ...
                <div className="aspect-[4/3] flex items-center justify-center bg-surface-2 dark:bg-surface-2">
                  <FileText size={36} className="text-subtle" />
                </div>
              )}
              ...
              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                {isImage(file.name) && (
                  <button
                    ...
                    className="w-9 h-9 rounded-[8px] bg-surface text-primary flex items-center justify-center hover:bg-surface-2 transition-colors cursor-pointer shadow-sm"
                    title="Padidinti"
                  >
                    <ZoomIn size={16} />
                  </button>
                ...
                {!isImage(file.name) && (
                  <a
                    ...
                    className="w-9 h-9 rounded-[8px] bg-surface text-primary flex items-center justify-center hover:bg-surface-2 transition-colors shadow-sm"
                    title="Atidaryti"
                  >
                    <ImageIcon size={16} />
                  </a>
