...
export default function ManualEntryModal({
  ...
  const inputCls =
    'w-full h-[42px] px-3 bg-zinc-50 dark:bg-surface-2 border border-transparent dark:border-white/10 rounded-xl text-[14px] text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary transition-all';
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-surface rounded-[20px] shadow-2xl w-full max-w-md border border-zinc-100 dark:border-white/10 flex flex-col max-h-[88vh]"
      >
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          ...
          <div>
            ...
            <div className="flex items-center gap-1 bg-zinc-100 dark:bg-surface-2 rounded-xl p-1">
              {TYPE_OPTIONS.map((o) => (
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg text-[13px] font-semibold transition-all cursor-pointer ${
                    type === o.value ? 'bg-white dark:bg-[#3f3f46] shadow-sm text-primary dark:text-primary-ink' : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200'
                  }`}
                >{o.label}</button>
              ))}
            </div>
