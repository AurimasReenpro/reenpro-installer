...
export default function ManualEntryModal({
  ...
  const inputCls =
    'w-full h-[42px] px-3 bg-surface-2 dark:bg-surface-2 border border-transparent dark:border-white/10 rounded-card text-[14px] text-text dark:text-white focus:outline-none focus:ring-2 focus:ring-primary transition-all';
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-surface rounded-[20px] shadow-2xl w-full max-w-md border border-border dark:border-white/10 flex flex-col max-h-[88vh]"
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-border dark:border-white/10 shrink-0">
          <h2 className="font-bold text-[16px] text-text flex items-center gap-2">
            <Coins size={17} className="text-primary dark:text-primary-ink" /> Naujas įrašas
          </h2>
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          {/* Installer */}
          <div>
            <label className="block text-[11px] font-bold text-subtle uppercase tracking-wider mb-1.5">Montuotojas</label>
          ...
          <div>
            ...
            <div className="flex items-center gap-1 bg-surface-2 dark:bg-surface-2 rounded-card p-1">
              {TYPE_OPTIONS.map((o) => (
                <button
          ...
          <div>
            ...
            <div className="relative">
              ...
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[13px] text-subtle pointer-events-none">€</span>
            </div>
            {type === 'deduction' ? (
              <p className="text-[11px] text-warning mt-1">Įveskite teigiamą sumą — bus išsaugota kaip neigiama.</p>
            ) : type === 'adjustment' ? (
              <p className="text-[11px] text-subtle mt-1">Korekcija gali būti teigiama arba neigiama.</p>
            ) : (
              <p className="text-[11px] text-subtle mt-1">Įveskite teigiamą sumą.</p>
            )}
          </div>

          {/* Description */}
          <div>
            <label className="block text-[11px] font-bold text-subtle uppercase tracking-wider mb-1.5">Aprašymas (privaloma)</label>
            <textarea
            ...
            />
            <p className={`text-[11px] mt-1 ${descValid || descTrimmed.length === 0 ? 'text-subtle' : 'text-danger'}`}>
          ...
          <div>
            <label className="block text-[11px] font-bold text-subtle uppercase tracking-wider mb-1.5">Susieti su objektu (nebūtina)</label>
            <select
