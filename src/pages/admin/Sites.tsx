...
const STATUS_CHIP: Record<string, { label: string; cls: string }> = {
  pending:     { label: 'Laukia',      cls: 'bg-[#F0F9FF] text-[#0284C7]' },
  in_progress: { label: 'Vykdomas',    cls: 'bg-[#ECFDF5] text-[#10B981]' },
  paused:      { label: 'Sustabdytas', cls: 'bg-[#FFFBEB] text-[#F59E0B]' },
  completed:   { label: 'Baigtas',     cls: 'bg-[#F3F4F6] text-[#6B7280] dark:bg-white/10' },
  archived:    { label: 'Archyvuotas', cls: 'bg-surface-2 text-subtle dark:bg-white/5' },
};
...
const selectCls = 'h-[40px] px-3 rounded-card bg-surface border border-border text-[14px] text-text focus:outline-none focus:ring-2 focus:ring-primary/20 cursor-pointer';
    ...
    <div className="space-y-5 h-full flex flex-col">
      ...
      <div className="flex justify-between items-start gap-4 flex-wrap">
        ...
        <button
          ...
          disabled={isCreating}
          className="flex h-10 items-center gap-2 whitespace-nowrap rounded-card bg-primary px-4 text-[14px] font-medium text-white shadow-sm transition-all hover:opacity-90 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60"
        >
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="relative flex-1 min-w-[220px] max-w-md">
          ...
          <input
            ...
            className="h-[40px] w-full rounded-card border border-border bg-surface pl-9 pr-3 text-[14px] text-text placeholder:text-subtle focus:outline-none focus:ring-2 focus:ring-primary/20"
          />
        </div>
        ...
        <button onClick={() => setF({ onlyNoTeam: !filters.onlyNoTeam })} className={`h-[40px] whitespace-nowrap rounded-card border px-3 text-[13px] font-medium transition-colors active:scale-[0.98] cursor-pointer ${filters.onlyNoTeam ? 'bg-warning-bg border-warning text-warning' : 'bg-surface border-border text-muted hover:bg-surface-2'}`}>
          Tik be komandos
        </button>
        <button onClick={() => setF({ onlyProblems: !filters.onlyProblems })} className={`h-[40px] whitespace-nowrap rounded-card border px-3 text-[13px] font-medium transition-colors active:scale-[0.98] cursor-pointer ${filters.onlyProblems ? 'bg-danger/10 border-danger text-danger' : 'bg-surface border-border text-muted hover:bg-surface-2'}`}>
          Tik su problemomis
        </button>
      </div>

      {/* ── Table ── */}
      <div className="bg-surface border border-border rounded-card shadow-sm dark:shadow-none flex-1 overflow-hidden flex flex-col">
              ...
              {isLoading ? (
                ...
                return (
                  <tr key={r.id} className={`border-b border-border dark:border-white/5 hover:bg-surface-2/50 dark:hover:bg-surface-2 transition-colors ${archived ? 'opacity-60' : ''}`}>
                    ...
                    <td className="py-3 px-4 font-semibold text-text max-w-[200px] truncate">{r.client_name}</td>
                    ...
                    <td className="py-3 px-4">
                      {r.teamName && !archived ? (
                        <span className="bg-[#f0fdf4] text-[#16a34a] border border-[#16a34a]/20 px-2.5 py-1 rounded-[6px] text-[12px] font-bold whitespace-nowrap">{r.teamName}</span>
                      ) : (
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2 text-[12px] text-muted">
                        {r.progress ? (
                          <span className={`inline-flex items-center gap-1 ${r.progress.failed > 0 ? 'text-danger' : ''}`}>
                    ...
                    <td className="py-3 px-4 text-[13px] whitespace-nowrap">
                      {r.time.activeMs != null ? (
                        <span className={`inline-flex items-center gap-1 ${r.time.suspicious ? 'text-danger' : 'text-success'}`}><Clock size={13} /> {fmtDuration(r.time.activeMs)}</span>
                      ) : r.time.totalHours != null ? (
                        <span className={`inline-flex items-center gap-1 ${r.time.suspicious ? 'text-danger' : 'text-muted'}`}>{r.time.totalHours} val.</span>
                    ...
                    <td className="py-3 px-4">
                      {r.warnings.length === 0 ? <span className="text-subtle">-</span> : (
                        <div className="flex flex-wrap gap-1 max-w-[220px]">
                          {r.warnings.map((w) => (
                            <span key={w} className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full bg-warning-bg text-warning border border-warning">
...
function Kpi({ label, value, tone = 'default', active, onClick }: { label: string; value: number; tone?: 'default' | 'sky' | 'emerald' | 'amber' | 'red' | 'zinc'; active?: boolean; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`text-left rounded-card border px-4 py-3 transition-all cursor-pointer ${active ? 'border-primary ring-1 ring-primary/30 bg-surface' : 'border-border bg-surface hover:bg-surface-2/50'}`}
    >
