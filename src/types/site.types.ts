...
export const getSiteDetailQuery = (id: string) => supabase
  .from('sites')
  .select('*, equipment_details, team:teams(name), time_entries(*), site_checklists(id, status, site_checklist_items(*)), photos(*), site_extra_materials(*)')
  .eq('id', id)
  .single();
...
export type SiteExtraMaterial = SiteDetailData['site_extra_materials'][number];
